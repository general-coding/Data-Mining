...
#             print(elevation_prob, aspect_prob, slope_prob, h_hydro_prob, v_hydro_prob, h_roadways_prob, 
#                      hillshade9am_prob, hillshadenoon_prob, hillshade3pm_prob, h_fire_prob, soil_prob, wilderness_prob)
            
            class_cover = float(elevation_prob) * float(aspect_prob) * float(slope_prob) * float(h_hydro_prob) * float(v_hydro_prob) * float(h_roadways_prob) * float(hillshade9am_prob) * float(hillshadenoon_prob) * float(hillshade3pm_prob) * float(h_fire_prob) * float(soil_prob) * float(wilderness_prob) * float(prob_cover[cover_type])
